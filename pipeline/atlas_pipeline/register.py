...
def icp(src_pts: np.ndarray, target: trimesh.Trimesh, T0: np.ndarray, iters=40, trim=0.9, z_clip=None) -> tuple[np.ndarray, float]:
    T = T0.copy()
    ...
    for _ in range(iters):
        ...
        if z_clip is not None:
            keep &= p[:, 2] > z_clip
        T = similarity_from_points(src_pts[keep], target.vertices[idx[keep]])
    p = src_pts @ T[:3, :3].T + T[:3, 3]
    d, _ = tree.query(p)
...
def main(argv=None) -> None:
    ...
    src = trimesh.util.concatenate([trimesh.load(str(WORK / "bp3d" / f"bp3d-cerebrum-{s}.ply"), force="mesh", process=True) for s in ("l", "r")])
    ...
    if a.refine and out_path.exists():
        ...
        best = icp(src_pts, target, T0, iters=80, trim=0.92)
    else:
        ...
        for perm in itertools.permutations(range(3)):
            for signs in itertools.product([1, -1], repeat=3):
                ...
                if np.linalg.det(R) < 0:
                    continue
                s = float(np.mean(tgt_ext / src_ext[list(perm)]))
                T0 = np.eye(4); T0[:3, :3] = s * R; T0[:3, 3] = tgt_c - s * R @ src_c
                T, rms = icp(src_pts, target, T0, iters=25, trim=0.9)
                if best is None or rms < best[1]:
                    best = (T, rms); print(f"  init perm={perm} signs={signs} rms={rms:.2f}")
        T, _ = icp(src_pts, target, best[0], iters=80, trim=0.92)
        best = (T, best[1])
    T = best[0]
    ...
    metrics = {"surface_rms_mm": round(rms, 2), "surface_p95_mm": round(p95, 2), "scale": round(scale, 4)}
    # landmarks
    lm = {}

...
export interface RouteParams { ax?: number; cor?: number; sag?: number; c?: 't1w' | 't2w' }
...
export function bindRouter(store: Store<AppState>, handlers: { onRoute(route: Route, params: RouteParams): void }): () => void {
  ...
  let timer = 0;
  const unsub = store.subscribe((s) => [s.selectedId, s.syndrome?.id ?? null, s.syndrome?.step ?? -1, s.slices.axial, s.slices.coronal, s.slices.sagittal, s.contrast] as const, (v) => {
    if (applying) return;
    clearTimeout(timer);
    timer = window.setTimeout(() => {
      const [sel, syn, step, ax, cor, sag, c] = v;
      const route: Route = syn ? { kind: 'syndrome', id: syn, step: step >= 0 ? step : undefined } : sel ? { kind: 'structure', id: sel } : { kind: 'slice' };
      const hash = serialize(route, { ax, cor, sag, c: c === 't2w' ? 't2w' : undefined });

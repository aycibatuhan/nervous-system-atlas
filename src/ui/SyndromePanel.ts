...
export class SyndromePanel {
  ...
  show(id: string): void {
    ...
    this.container.append(h('div', {},
      ...
      h('h3', {}, enTag(this.current), t('syndrome.deficits')),
      ...
      h('h3', {}, enTag(this.current), t('syndrome.imaging')), h('table', { class: 'tbl' }, h('tr', {}, h('th', {}, t('th.pathology')), h('th', {}, t('th.modality')), h('th', {}, t('th.finding'))),
      ...
      h('h3', {}, enTag(this.current), t('syndrome.causes')), h('ul', {}, ...((syn['commonCauses'] as string[]) ?? []).map((x) => h('li', {}, x))),
      h('h3', {}, enTag(this.current), t('syndrome.mimics')), h('ul', {}, ...((syn['mimics'] as Rec[]) ?? []).map((m) => h('li', {}, h('b', {}, String(m['name'])), ` — ${m['howToDistinguish']}`))),
      ((syn['examSequence'] as string[]) ?? []).length ? h('div', {}, h('h3', {}, enTag(this.current), t('syndrome.examSequence')), h('ol', {}, ...(syn['examSequence'] as string[]).map((x) => h('li', {}, x)))) : null,
      h('h3', {}, enTag(this.current), t('syndrome.management')), h('ul', {}, ...((syn['management'] as string[]) ?? []).map((x) => h('li', {}, x))),
      h('h3', {}, t('syndrome.sources')), h('ul', {}, ...((syn['citations'] as unknown as Citation[]) ?? []).map((c) => h('li', {}, this.cite(c)))),
    ));
  }
}
